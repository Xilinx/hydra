...
(
  ...
  generate
    if ( C_nR == 1 )
        assign condition = S_AXIS_0_tdata[C+ID] ? 1'b1 : 1'b0;
    else
        assign condition = S_AXIS_0_tdata[R+ID] ? 1'b1 : 1'b0;
  endgenerate

  assign M_AXIS_0_tdata = S_AXIS_0_tdata;
  assign M_AXIS_1_tdata = S_AXIS_0_tdata;

  assign M_AXIS_0_tvalid = condition ? S_AXIS_0_tvalid : 1'b0; //condition 1 -- go right
  assign M_AXIS_1_tvalid = condition ? 1'b0 : S_AXIS_0_tvalid;
